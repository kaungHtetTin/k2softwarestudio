...
<div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
    <div>
        ...
        <h2 class="h4 mb-0 text-dark">App gallery</h2>
    </div>
    <a class="btn btn-k2-accent" href="<?= k2_e(k2_url('/admin/apps/new')) ?>">
        <i class="bi bi-plus-lg me-1" aria-hidden="true"></i> New app
    </a>
</div>
...
<?php if ($rows === []) : ?>
    <p class="text-muted">No apps yet. Add one to show it on the public gallery.</p>
<?php else : ?>
                ...
                <?php foreach ($rows as $r) :
                    ...
                    <tr>
                        ...
                        <td class="text-end text-nowrap">
                            <a class="btn btn-sm btn-outline-primary" href="<?= k2_e(k2_url('/admin/apps/edit') . '?id=' . $rid) ?>">Edit</a>
                            <form method="post" action="<?= k2_e(k2_url('/admin/apps/delete')) ?>" class="d-inline" onsubmit="return confirm('Delete this app and all screenshots?');">
                                <?= k2_csrf_field() ?>
