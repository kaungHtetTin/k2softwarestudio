...
<div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
    <div>
        ...
        <h2 class="h4 mb-0 text-dark">FAQ</h2>
    </div>
    <a class="btn btn-k2-accent" href="<?= k2_e(k2_url('/admin/faq/new')) ?>">
        <i class="bi bi-plus-lg me-1" aria-hidden="true"></i> New question
    </a>
</div>
...
<?php if ($rows === []) : ?>
    <p class="text-muted">No FAQ entries yet. Add questions shown on the home page (#faq).</p>
<?php else : ?>
        ...
        <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
                <?php foreach ($rows as $r) :
                    ...
                    ?>
                    <tr>
                        <td class="fw-medium"><?= k2_e((string) ($r['question'] ?? '')) ?></td>
                        <td>
                            <?php if ($vis) : ?>
                        ...
                        <td class="text-end text-nowrap">
                            <a class="btn btn-sm btn-outline-primary" href="<?= k2_e(k2_url('/admin/faq/edit') . '?id=' . $rid) ?>">Edit</a>
                            <form method="post" action="<?= k2_e(k2_url('/admin/faq/delete')) ?>" class="d-inline" onsubmit="return confirm('Delete this FAQ entry?');">
                                <?= k2_csrf_field() ?>
